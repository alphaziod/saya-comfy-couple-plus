...
# Saya patch: pad different CLIP context lengths before torch.cat
def saya_pad_context_tensors(tensors):
    tensors = list(tensors)
    if len(tensors) < 2:
        return tensors

    max_tokens = max((t.shape[1] for t in tensors if hasattr(t, "ndim") and t.ndim >= 3), default=None)
...
def saya_safe_cat_context(tensors, dim=0):
    return torch.cat(saya_pad_context_tensors(tensors), dim=dim)
...
class SayaCoupleAttn2Replace:
    def __init__(self, couple_patch):
...
def set_model_patch_replace(model, patch, key):
    ...
    to["patches_replace"]["attn2"][key] = SayaCoupleAttn2Replace(patch)
...
class AttentionCouple:
    ...
    def attention_couple(self, model, positive, negative, mode):
        ...
        for conditions in [new_negative, new_positive]:
            ...
            if len(conditions) != 1:
                ...
                mask_norm = torch.stack([cond[1]["mask"].to(device, dtype=mask_dtype) * cond[1]["mask_strength"] for cond in conditions])
    ...
    def make_patch(self, module):
        def patch(q, k, v, extra_options):
            ...
            context_uncond = saya_safe_cat_context(self.negative_positive_conds[0], dim=0)
            context_cond = saya_safe_cat_context(self.negative_positive_conds[1], dim=0)
